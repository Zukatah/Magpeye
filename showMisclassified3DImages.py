...
#with open('NotesAndTools/CorrectlyLabeledFiles.csv', 'r') as csvfile:
    ...
    for (i, row) in enumerate(notesReader):
        if len(row) == 7:
            ...
            #newimg1 = cv.hconcat([img[i,:,:] for i in range(15)])
            #newimg2 = cv.hconcat([img[i+15,:,:] for i in range(15)])
            #newimg = cv.vconcat([newimg1, newimg2])
            newimg = cv.hconcat([img[i,:,:] for i in range(TRAINING_EXAMPLE_DEPTH)])
...
'''
folderpath = "Pictures3D_NewlyCreated/train/2/"
